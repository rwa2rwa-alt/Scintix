/* ════════════════════════════════════════════════════════════════
   ...
   ما تغيّر (2026-08-18):
   5) سطر واحد لكل عملية لدى ميسر، لا سطر لكل استدعاء. كانت الدالة
      تُنشئ سجلاً جديداً في كل مرة تُستدعى، فتحديث صفحة النتيجة أو
      رجوع المتصفح يكرّر القيد بنفس Gateway Ref ويضخّم الإيرادات.
      الآن تبحث أولاً عن سطر بنفس المرجع والاتجاه: إن وُجد بنفس
      الحالة تنصرف، وإن تغيّرت الحالة (قيد المراجعة ← مؤكدة) تُحدّثه
      بدل إنشاء سطر ثانٍ.
   ════════════════════════════════════════════════════════════════ */
...
/* ─────────────────────────────────────────────────────────────────
   قيد سطر في سجل الحركات — سطر واحد لكل عملية لدى ميسر.

   المفتاح المنطقي هو (Gateway Ref + Direction). ميسر يعطي كل عملية
   معرّفاً فريداً، فالمعرّف نفسه مرتين يعني استدعاءً مكرراً لا دفعة
   ثانية. الاسترداد اتجاه مختلف فيأخذ سطره الخاص بحق.
...
async function logPayment(atHeaders, p) {
  ...
  const direction = p.direction || 'تحصيل';
  const stamp     = new Date();

  try {
    /* ─ هل قُيّدت هذه العملية من قبل؟ ─────────────────────────── */
    const existing = await findPayment(atHeaders, p.paymentId, direction);

    if (existing) {
      /* نفس الحالة = استدعاء مكرر بحت. لا تلمس السجل. */
      if ((existing.fields || {})['Status'] === p.status) {
        console.log('دفعة مقيّدة مسبقاً، تخطّي:', p.paymentId, p.status);
        return;
      }

      /* الحالة تغيّرت (قيد المراجعة ← مؤكدة مثلاً): حدّث السطر نفسه.
         Payment No وDate يبقيان كما هما — القيد الأصلي لا يُزوَّر. */
      await patch(atHeaders, `${AT}/${BASE_ID}/${PAYMENTS}/${existing.id}`, {
        typecast: true,
        fields: {
...
/* ─────────────────────────────────────────────────────────────────
   البحث عن قيد سابق بنفس مرجع البوابة والاتجاه.
   يعيد null عند أي خطأ — لأن تعذّر البحث يجب ألا يمنع القيد.
   ───────────────────────────────────────────────────────────────── */
async function findPayment(atHeaders, paymentId, direction) {
  /* اقتباس آمن داخل صيغة Airtable: المرجع من ميسر لا يحوي علامات
     اقتباس، لكن لا نبني صيغة من مدخل خارجي دون تنظيف. */
  const q = (v) => '"' + String(v).replace(/["\\]/g, '') + '"';
  const formula = `AND({Gateway Ref}=${q(paymentId)},{Direction}=${q(direction)})`;

  try {
    const res = await fetch(
      `${AT}/${BASE_ID}/${PAYMENTS}?filterByFormula=${encodeURIComponent(formula)}&maxRecords=1`,
      { headers: atHeaders }
    );
    if (!res.ok) {
      console.error('البحث عن قيد سابق فشل', res.status);
      return null;
    }
    return ((await res.json()).records || [])[0] || null;
  } catch (e) {
    console.error('البحث عن قيد سابق فشل:', e && e.message);
    return null;
  }
}
